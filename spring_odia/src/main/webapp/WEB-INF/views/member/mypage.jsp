...
<body>
	<form class="body container" method="post" action="<%=request.getContextPath()%>/mypage">
		...
		<div class="form-group">
			<div class="form-check-inline">
				<label class="form-check-label">
					<input type="radio" class="form-check-input" name="me_gender" value="남성"
						<c:if test="${user.me_gender == '남성'}">checked</c:if>>남성
				</label>
			</div>
			<div class="form-check-inline">
				<label class="form-check-label">
					<input type="radio" class="form-check-input" name="me_gender" value="여성"
						<c:if test="${user.me_gender == '여성'}">checked</c:if>>여성
				</label>
			</div>
		</div>
		...
		<button class="btn btn-outline-success btn-mod col-12">회원정보 수정</button>
	</form>
...
$('.btn-mod').click(function(){
	...
	if(me_address == '' || me_pw == ''){
		...
		//prompt는 문자열을 입력받은 입력창으로 리턴값이 입력한 문자열이 된다.
		return confrim('비밀번호 또는 주소를 입력하지 않은 경우 이전 정보로 저장됩니다. 계속하시겠습니까?')
	}
