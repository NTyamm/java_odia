...
public class ExaStringSplitEx1 {
	...
	public static void main(String[] args) {
		 ...
		 * 	suffix로 끝나면 true, 아니면 false를 사용(리턴타입 boolean)*/
		 ...
		 * -문자열 시작부분이 prefixc로 시작하는지 알려주는 메소드
		 *  prefix로 시작하면 true, 아니면 false를 알려줌*/
		String fileName[] = new String[2];
		fileName[0] = "ExaStringReplaceEx1.jave";
		fileName[1] = "ExbStringSplitEx1.jave";
		for(String tmp : fileName) {
			if(tmp.startsWith("Exa")) {
				System.out.println(tmp +"는 첫번째 예제들입니다.");
			}else {
				System.out.println(tmp + "는 첫번째 예재가 아닙니다.");
			}
		}
		/* \n: dpsxj, \t: 탭키를 누른 공백(일반적으로 4칸) */
		String strTrim ="\n\t\tabc\n\nabc\n\n";
