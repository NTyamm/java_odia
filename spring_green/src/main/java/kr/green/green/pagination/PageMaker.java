...
public class PageMaker {
	...
	public void calcData() {
		/* starPage와 endPage는 현재 페이지 정보인 criteria와 displayPageNum을 이용하여 계산. 이렇게 정해져있음
		 * displayPageNum이 10이고 현재 페이지가 3페이지면 startPage = 1, endPage = 10이 되도록 계산 */
		endPage = (int) (Math.ceil(criteria.getPage()/(double) displayPageNum)*displayPageNum);
		
		startPage = (endPage - displayPageNum)+1;
		/* 총 콘텐츠 갯수를 이용하여 마지막 페이지네이션에서 마지막 번호를 계산
		 * (현재 게시글 수에서 만들 수 있는 가장 마지막 페이지) */
		...
		/* 페이지네이션 설정에 따라 달라질 수 있다.(이전/다음 버튼으로 페이지네이션을 이동할 것인지 다음 페이지로 갈 것인지 등)
		 * 다음 페이지 클릭하면 현재 페이지에서 다음 페이지로 이동
		 * 현재 페이지가 1페이지면 다음 버튼을 클릭하면 2페이지로 이동
		 * 이전 버튼을 클릭하면 현재 페이지에서 이전 페이지로 이동
		 */
		//위 설정에서 이전 버튼은 1페이지에만 비활성화시키면 됨. 
		prev = criteria.getPage() == 1 ? false : true; 
		//start페이지가 1페이지면 이전 버튼을 비활성화
		// prev = startPage == 1 ? false : true;
		//위 설정에서 다음 버튼은 마지막 페이지에서만 비활성화시키면 됨
