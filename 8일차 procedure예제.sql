...
- 단점
 1. 재사용성이 나쁘다.
 2. 업무의 사양 변경시 프로시저의 정의를 변경할 필요가 있다.
- 프로시저 목록 확인
...
- 프로시저 정의
 ...
 (
	[(in|out) 변수명 타입,
    ...
    (in|out) 변수명 타입] -- out은 여러개 있을 수 있다.
 )
 begin
 -- 실행코드
 end //
...
-- 과일 판매시 판매 금액을 자동으로 입력하여 판매 정보를 sell 테이블에 추가하는 프로시저 
 ...
 begin
    ...
    insert into sell
		select max(se_num)+1, in_fr_name, in_amount, in_unit, _total_price, now(), int_type
			from sell;
 end //
 delimiter; -- why??? 
...
통신사에서 작년 사용 금액을 기준으로 등급을 결정하는 과정을 하려 한다.
이 때 프로시저? 트리거?  -> 프로시저
제품을 구매할 때마다 구매 금액의 10%를 포인트로 지급하려 한다.
이 때 프로시저? 트리거 -> 트리거
*/